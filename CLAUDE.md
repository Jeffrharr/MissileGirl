...
| `--expect-typeprovider` | **PR 1 for issue #86** (custom Def-type provider removal invalidates dependent defs across other mods). `joof.testharness.typeprovider` ships a C# assembly defining `JoofTest.GadgetDef : Def` and nothing else; active for Run A, REMOVED before Run B (same toggle as `--expect-mayrequire`'s gate mod). `joof.testharness.typeconsumer` (UNCHANGED both runs) declares 3 unconditional, unpatched `<JoofTest.GadgetDef>` instances — no patches, no MayRequire. The provider's removal makes the .NET Type disappear, so Seed 9 (`TypeProviderIndex` XOR on packageId presence) must dirty all 3 consumer defs even though the consuming mod never changed. This is the real `oppey.eyegenes2`/`nals.facialanimation` shape (`FacialAnimation.EyeballColorDef`), reproduced with a throwaway type | dirty-set gate + `seeds.typeProvider > 0` in `DirtySet.json` — live-validated 2026-07-17: `seeds.typeProvider=3`, `nonDirtyMismatches=0`. Recompute is informational (fails as expected: no `TypeProviderGate`/`DefRecompute` step yet — that's PR 2, mirroring how `--expect-mayrequire` started) |
...
Recompute gate is `RecomputeReport.json` (required except in `--expect-mayrequire` and, for now,
`--expect-typeprovider` — see issue #86 PR 2 in the roadmap).
