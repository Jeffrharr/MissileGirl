Reference /home/deck/Developer/RimWorldMods/CLAUDE.md before this file for general rimworld project instructions
...
`GAGARIN_DIRTYSET_RECOMPUTE`, `GAGARIN_TYPEPROVIDER_RECOMPUTE` (issue #86 PR 2 recompute fidelity,
not yet live-validated), `GAGARIN_METRICS`.
...
| `--expect-nested-conditional` | CASE 7 (issue #25 / PR #37): a conditional (cross-def test on `TC_NestedProbe`) nested inside another conditional's branch (same-def test on `TC_NestedTarget`, unchanged `TestMod_Static`); `TestMod_Change` dirties `TC_NestedTarget` (Seed 2) | dirty-set gate + `fallback==true && pass==true` with a conditional-cross-def reason — proves `RecomputeAllowlist.BranchParentId` resolves the leaf's **immediate** (inner) gating conditional, not the outer one |
| `--expect-nested-in-sequence` | CASE 8 (issue #25 item 3): a conditional (cross-def test on `TC_SeqNestedProbe`) nested inside a `PatchOperationSequence`'s `<operations>` list (unchanged `TestMod_Static`); `TestMod_Change` dirties `TC_SeqNestedTarget` (Seed 2) | dirty-set gate + `fallback==true && pass==true` **plus** a direct `DependencyGraph.json` read asserting a `patchEdge` with `patchId` ending `.operations[0]` has a non-empty `matchedNodeIds` — proves CAPTURE (not just the allowlist decision) records a conditional's own test edge under the `.operations[i]` id-label path, not just the `.match`/`.nomatch` path CASE 7 covers |
| `--expect-op-kind` | **Positive regression, PASSES live (validated 2026-07-09).** CASE 10: `PatchOperationFindMod` (targeting "Harmony", always-active) producing `TC_OpKind_Target`; `TestMod_Change` dirties it directly (Seed 2). `PatchOperationFindMod` is now in `SafeLeafOps` (fully-qualified as `Verse.PatchOperationFindMod`) | dirty-set gate + **default** recompute gate (`fallback==false && recomputeMismatches==0`) — was the XFAIL worklist item; closed once FindMod was proven safe and added to `SafeLeafOps` (offline-pinned by `RecomputeAllowlistTests.FindMod_ShouldBeAdmitted_OnceProvenSafe`) |
| `--expect-order-preserved` | Positive regression, expected to PASS. CASE 11: two same-def ops on `TC_Order_Source` in file order (unconditional Add setting `orderFlag=on`, then a `PatchOperationConditional` reading it); `TestMod_Change` dirties it directly (Seed 2) | dirty-set gate + **default** recompute gate (`fallback==false && recomputeMismatches==0`) — pins `DefRecompute`'s ordering guarantee for same-def ops actually present in the sub-doc |
| `--expect-scope-escape` | Positive regression for a review-flagged gap in `RecomputeAllowlist.IsUnsafePositional`. CASE 12: `TestMod_Static` carries a `PatchOperationReplace` anchored on `TC_ScopeEscape_A`'s defName but escaping via `following-sibling::ThingDef[1]` to the adjacent `TC_ScopeEscape_B` — a lexically-earlier anchor that does NOT structurally scope the positional step; `TestMod_Change` dirties `TC_ScopeEscape_B` directly (Seed 2) | dirty-set gate + `fallback==true && pass==true && recomputeMismatches==0` with a `positional-xpath` fallback reason — proves the structural per-step parser (not the old string-position heuristic) declines the escape; offline-pinned by `RecomputeAllowlistTests.PositionalXpath_SiblingAxisAfterAnchor_Declined` / `PositionalXpath_AfterScopeBreak_Declined` |
| `--expect-rogue-op` | Live proof of the fully-qualified `SafeLeafOps` sweep (the original CRITICAL spoofing gap). CASE 13: `TestMod_RogueOp` ships its own class literally named `PatchOperationAdd` (namespace `RogueMod`, not `Verse`), applied via `Class="RogueMod.PatchOperationAdd"`, always producing `TC_RogueOp_Target`; `TestMod_Change` dirties it directly (Seed 2) | dirty-set gate + `fallback==true && pass==true && recomputeMismatches==0` with a fallback reason naming `RogueMod.PatchOperationAdd` — proves `SafeLeafOps` is checked against `Type.FullName`, not the bare simple name that would collide with `Verse.PatchOperationAdd` |
| `--expect-patch-injected-child` | Positive regression for `DefRecompute.BuildTopLevelIdsToRun`, found via the `check-my-vibe` PR #62 interview. CASE 14: `TestMod_Static` (UNCHANGED) injects a brand-new top-level def, `TC_PatchInjected_Child`, via a `PatchOperationAdd` targeting the `Defs` root — that child has no raw body and `ParentName="TC_PatchInjectedBase"`; `TestMod_Change` dirties `TC_PatchInjectedBase` directly (Seed 2), and inheritance-closure fan-out dirties the child too even though its only owner never changed. Before the fix, the unchanged-mod top-level filter could never mark the Add op relevant (its captured edge records the `Defs`-root anchor, never the child's own id) and `DefRecompute` wrongly recomputed the child as deleted; the fix falls back to `patchInjectedOwners` (recorded at capture time) to keep the op in the replay set whenever it owns a `pendingUnresolved` id | dirty-set gate + **default** recompute gate (`fallback==false && recomputeMismatches==0`) — same shape as `--expect-order-preserved` |
| `--expect-typeprovider` | **PR 1 + PR 2 for issue #86** (custom Def-type provider removal invalidates dependent defs across other mods). `joof.testharness.typeprovider` ships a C# assembly defining `JoofTest.GadgetDef : Def` and nothing else; active for Run A, REMOVED before Run B (same toggle as `--expect-mayrequire`'s gate mod). `joof.testharness.typeconsumer` (UNCHANGED both runs) declares 3 unconditional, unpatched `<JoofTest.GadgetDef>` instances — no patches, no MayRequire. The provider's removal makes the .NET Type disappear, so Seed 9 (`TypeProviderIndex` XOR on packageId presence) must dirty all 3 consumer defs even though the consuming mod never changed. This is the real `oppey.eyegenes2`/`nals.facialanimation` shape (`FacialAnimation.EyeballColorDef`), reproduced with a throwaway type | dirty-set gate + `seeds.typeProvider > 0` in `DirtySet.json` — live-validated 2026-07-17: `seeds.typeProvider=3`, `nonDirtyMismatches=0`. Recompute fidelity (`TypeProviderGate`, mirroring `MayRequireGate`) landed offline-tested, wired behind its own flag (`GagarinPrefs.TypeProviderRecompute` / `GAGARIN_TYPEPROVIDER_RECOMPUTE`, default OFF — unlike `MayRequireGate` it isn't unconditional in `DefRecompute` yet, since it's not yet live-validated) but is **not yet live-validated** — `run_test.sh` exports the flag on for every mode, so the next live run should confirm `recomputeMismatches==0`, at which point the flag can fold into the unconditional path (like `MayRequireGate`) and this row can flip out of the `--expect-mayrequire`-style exemption below |
...
Recompute gate is `RecomputeReport.json` (required except in `--expect-mayrequire`, and
`--expect-typeprovider` pending its first live run with the new `TypeProviderGate` — see roadmap).
