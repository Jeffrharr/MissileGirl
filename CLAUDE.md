...
| `--expect-op-kind` | **XFAIL worklist item, expected to currently FAIL.** CASE 10: `PatchOperationFindMod` (targeting "Harmony", always-active) producing `TC_OpKind_Target`; `TestMod_Change` dirties it directly (Seed 2) | dirty-set gate + **default** recompute gate (`fallback==false && recomputeMismatches==0`) — asserts the DESIRED end state (a real recompute), which `RecomputeAllowlist` currently declines as `unknown-op-kind` (offline-pinned by `RecomputeAllowlistTests.FindMod_ShouldBeAdmitted_OnceProvenSafe`, itself currently red); passes once FindMod is proven safe and added to `SafeLeafOps` |
