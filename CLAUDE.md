...
`GAGARIN_DIRTYSET_RECOMPUTE`, `GAGARIN_METRICS`. (`TypeProviderGate` — issue #86 PR 2 recompute
fidelity — folded into the unconditional recompute path once live-validated; no longer has its own
flag, mirroring `MayRequireGate`.)
...
Recompute gate is `RecomputeReport.json` — required for every `--expect-*` mode per its own
criteria in the table above (both `--expect-mayrequire` and `--expect-typeprovider` used to be
exemptions pending recompute-fidelity work; both are now live-validated and required).
