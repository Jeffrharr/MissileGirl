...
namespace Gagarin.Tests
{
    ...
    public class RecomputeAllowlistTests
    {
        ...
        // Issue #75 / check-my-vibe interview (PR #78): ComputeAncestorDivergence is the pure
        // set-subtraction DirtySetGate.RunRecompute uses to cross-check DefRecompute's current-raw-XML
        // ancestor walk against CanRecompute's relevantTargets. Pulled out so it's offline-testable
        // instead of only reachable through the RimWorld-coupled recompute path.
        [Test]
        public void ComputeAncestorDivergence_IdAbsentFromRelevantTargets_IsReported()
        {
            var relevantTargets = new HashSet<string>(new[] { "ThingDef/Parent" });
            List<string> divergence = RecomputeAllowlist.ComputeAncestorDivergence(
                new List<string> { "ThingDef/Parent", "ThingDef/OtherAncestor" }, relevantTargets);

            Assert.That(divergence, Is.EquivalentTo(new[] { "ThingDef/OtherAncestor" }));
        }

        // Every raw-XML ancestor id is also in relevantTargets -> no divergence to report.
        [Test]
        public void ComputeAncestorDivergence_AllIdsPresentInRelevantTargets_IsEmpty()
        {
            var relevantTargets = new HashSet<string>(new[] { "ThingDef/Parent", "ThingDef/Grandparent" });
            List<string> divergence = RecomputeAllowlist.ComputeAncestorDivergence(
                new List<string> { "ThingDef/Parent", "ThingDef/Grandparent" }, relevantTargets);
        ...
        public void PatchInjectedAncestor_NoSourceFile_AbsentFromPatchInjectedOwners_ShouldDecline_OnceGapClosed()
        {
            var g = Graph(Edge("mod#0", "Verse.PatchOperationReplace",
                "Defs/ThingDef[defName=\"Child\"]/label", "ThingDef/Child"));
            Inherit(g, "ThingDef/InsertedAncestorBase", "ThingDef/Child");
            // No PatchEdges reference the ancestor's own id at all -- its creating op's captured edge
            // (if any) named the Defs-root anchor it inserted at, never the new base's own id.
            g.Nodes.Add(new GraphNode
            {
                Id = "ThingDef/InsertedAncestorBase", DefType = "ThingDef", DefName = null,
                SourceMod = "mod", SourceFile = null,
            });
