...
| 2026-07-10 | 2 | seed 6003 (45-mod curated base), REMOVE 6 / ADD 7 | N (inconclusive, abandoned) | Run A's ModsConfig ended up with none of the 6 intended REMOVE-list mods active at all; user observed an incompatible-mods condition in the RimWorld window during Run B and closed it manually before the recompute-gate marker appeared. Harness's own teardown had already restored ModsConfig/DLL/symlinks cleanly. | — | Not counted against the streak (mod-collision class, not a Gagarin/incremental gap — see "Known issues"). Candidate abandoned, drew a fresh seed rather than re-chasing this combo. |
| 2026-07-10 | 2 | seed 7211 (41-mod curated base, dependency-closed), REMOVE 6 (`ferny.moveyourmonolith`, `VanillaExpanded.VAnomalyEInsanity`, `VanillaExpanded.VanillaTradingExpanded`, `VanillaExpanded.HelixienGas`, `Pacca.GeneVoiceAnimalsVanilla`, `VanillaExpanded.VCookEBakery`), ADD 6 (`Mlie.DingOnGameLoaded`, `Owlchemist.MidSaverSaver`, `MemeGoddess.RunAndGun`, `MRK.architectmenuoptimizer`, `Memegoddess.MealsOnWheels`, `ferny.progressionsoundtrack`) | N | — | — | **PASS**: dirty-set gate `pass=True nonDirtyMismatches=0` (dirtyCount=467/16878); recompute gate `pass=True fallback=True recomputeMismatches=0` — legitimate safe fallback (`vanillaexpanded.vanillatradingexpanded#0.match.operations[0]` is `PatchOperationAddModExtension`, not a proven-safe leaf op), not a defect. Streak now 6. |
